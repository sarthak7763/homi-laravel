        ...
        <div class="main-body">
            <div class="page-wrapper">
                ...
                <div class="page-header my-2">
                    ...
                    <a class="btn btn-primary btn-round" href="{{route('admin-category-add')}}">Add Category</a>
...
$('.datepicker').datepicker({
  dateFormat: 'yy-mm-dd'
    ...
    $(document).on('click', '.badge_category_status_change', function(e)
    { 
            var status_class = $(this).attr('class');
            var id = $(this).attr('id');
            var change_btn = $(this);
            var url = "{{ route('admin-category-status-update') }}";
            
            if(status_class == "badge badge-danger badge_category_status_change")
            {
                var newClass = "badge badge-success badge_category_status_change";
                var status = 'Active';
            }else
            {
                var newClass = "badge badge-danger badge_category_status_change";
                var status = 'Inactive';
            }

           var title ='Are you sure to '+status+' this Category ?';
            e.preventDefault();      
            swal({
              title: title,
              icon: "warning",
                ...
                $.ajax({
                    ...
                    success: function (data){
                        if(data.success==1){
                            ...
                            if(status=="Active"){
                             toastr.success("Category status active successfully");
                            }else{
                                  toastr.error("Category status inactive successfully");
                            }
            ...
            // else {
            //     swal("Cancelled", "Category status not change", "error");
            // }
            });         
    }); 


    $(document).on('click', '.badge_delete_category_status_change', function(e)
    { 
            var status_class = $(this).attr('class');
            var id = $(this).attr('id');
            var change_btn = $(this);
