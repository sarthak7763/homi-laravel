...
@section('title', 'Update Mail Template')
@section('content')
<div class="card">
    <div class="card-header">
        <h5>Add Mail Params and Type for Mail Template use</h5>
        <span>This page create type of mail and dynamic mail variables to use for mail templates</span>
        <a href="{{route('admin-mail-template-list')}}" class="btn btn-info pull-right">Go Back to mail template Page</a>
...
<button type="button" class="btn btn-success" id="button">Add New Row</button>
        </div>
    </div>
</div>
@endsection
...
<script type="text/javascript">
  ...
  table.append($('<tr><td>'+nextId+'</td><td></td><td></td></tr>'));
...
(function($) {
    ...
    $.Tabledit = function(element, options) {
        ...
        var defaults = {
            url: '{{route("admin-mail-action-data-save")}}',
            ...
            columns: {
                identifier: function() {
                    ...
                    $td.each(function() {
                        ...
                        var input = '<input class="tabledit-input tabledit-identifier" type="hidden" name="' + settings.columns.identifier[1] + '" value="' + $(this).text() + '" disabled><span id="'+ settings.columns.identifier[1] + '">';
                    ...
                    for (var i = 0; i < settings.columns.editable.length; i++) {
                        ...
                        $td.each(function() {
                            ...
                            if (typeof settings.columns.editable[i][2] !== 'undefined') {
                                ...
                                var input = '<input class="tabledit-input ' + settings.inputClass + '" type="text" required="required" name="' + settings.columns.editable[i][1] + '" value="' + $(this).text() + '" style="display: none;" disabled>';
                            }
        ...
        function ajax(action)
        {
            var serialize = $table.find('.tabledit-input').serialize() + '&action=' + action;
               var serialize = $table.find('.tabledit-input').serialize() + '&action=' + action + '&_token=' + csrf;
