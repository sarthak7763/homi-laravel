  ...
  public function searchProperty(){
    if(Auth::user()){
     ...
     $stateList = DB::table('properties')
        ->selectRaw('properties.id as propertyID,count(properties.id) as count_property,cities.*,cities.id as cityID,cities.name as cityName,states.name as stateName,states.id as stateID,property_offers.date_start,property_offers.sale_status')
        ->join('cities', 'properties.city', '=', 'cities.id')
        ->join('states', 'cities.state_id', '=', 'states.id')
        ->join('property_offers', 'property_offers.property_id', '=', 'properties.id')
        ...
        ->where('states.delete_status',0)
         ->where('property_offers.date_start','<=',$currentdate)
         ->where('property_offers.sale_status',1)
        ->whereIn('properties.escrow_status',["Active","Pending"])
        ->groupBy('properties.city')
        ->orderBy('cities.name')
        ->get();

//         $sql = Str::replaceArray('?', $stateList->getBindings(), $stateList->toSql());
// echo $sql;die;
    ...
    if(Auth::user()){
      ...
      if(!$property->isEmpty()){
        ...
        if(!$property->isEmpty()){
          ...
          $propertyInfo=Property::with(['favProperty' => function ($hasMany) {
              $hasMany->where('buyer_id', @Auth::user()->id);
            }])->where('escrow_status',"Active")->where([['status',1],['delete_status',0]])->orderBy('base_price','DESC')->get()->toArray();;
        }else{
          return response()->json(["status" => "error","mapData"=>[],"result"=>[],"result_city"=>[],"result_count"=>0,'property_counter'=>[]]);
        }
      }
      ...
      $pending_count=0;
      if($result_count > 0){
        foreach($propertyInfo as $key => $li){
        $currentdates = date('Y-m-d');
          $propertyListoffer = DB::table('property_offers')      
         ->where('property_id',  $li['id'])
        ->where('sale_status',1)
        ->where('date_start', '<=',  $currentdates)
         ->orderBy('id')->get()->first();

        if($propertyListoffer){
        if($li['id'] == $propertyListoffer->property_id)
         {
           if($li['escrow_status']=="Active"){
            $active_count++;
          }else if($li['escrow_status']=="Pending"){
          ...
          // echo $key;
          // print_r($propertyInfo[$key]);    
           unset($propertyInfo[$key]); 
         }
        ...
        if(isset($propertyInfo) && (!empty($propertyInfo))){
          foreach($propertyInfo as $key=>$pli)  {
            if(!empty($pli['fav_property'])){
              $fav=1;
              $fav_id=$pli['fav_property'][0]['id'];
            }else{
            ...
            $propertyInfo[$key]['fav']=$fav;
            $propertyInfo[$key]['fav_id']=$fav_id;
          }
        }     
      } 
     ...
     $result_count=count($propertyInfo);
      return response()->json(["status" => "success","mapData"=>$searchProperty,"result"=>$propertyInfo,"result_city"=>$result_city,"result_count"=>$result_count,"property_counter"=>$property_counter]);
    }else{
      return redirect()->route('buyer-login');
    }
  }
  ...
  public function searchPropertyCityWise(Request $request){
   // print_r('sdfsf');die;
    ...
    if(Auth::user()){
      ...
      if(!$property->isEmpty()){
        $property=$propertyInfo->get()->toArray();
        $result_count=count($property); 
        $active_count=0;
        $pending_count=0;
        if($result_count > 0){
          foreach($property  as $key => $li){
        ...
        if($li['id'] == $propertyListoffer->property_id)
         ...
         if($li['escrow_status']=="Active"){
          ...
          foreach($property as $key=>$pli){
            ...
            $property[$key]['fav']=$fav;
            $property[$key]['fav_id']=$fav_id;
          }
        }

        return response()->json(["status" => "success","mapData"=>$searchProperty,"result"=>$property,"result_city"=>$result_city,"result_count"=>$result_count,"property_counter"=>$property_counter]);
